...
%% affinity of each node
[~, ~, ~, ~, rk] = graphAgglomerativeClustering(distance_matrix, clusterNum, 'gdl', K, 0, 1, false, X);
figure(51)
scatter(X(:,1), X(:,2), 20, rk, 'filled'); 
axis([0 im_N 0 im_M]);
set(gca,'YDir','Reverse')
title('Affinity of each node')
grid on;
...
X_sub = X(chosen_idx, :);
% distance_matrix_sub = distance_matrix;
% X_sub = X;
...
% K = 10;
a = 1;
% last para -- false: no normalization; true: normalization
[clusteredLabels, Q, W, cost, rk_2] = graphAgglomerativeClustering(distance_matrix_sub, clusterNum, 'gdl', K, 1, a, false, X_sub, true);
